...
| 2 | **MVP-001 Factory Agent/tool vertical slice** | Implement only the authoritative frozen Day-10 scope in `plans/day10_mvp_scope_v1.md`. It may begin immediately after P0-003 and must not wait for VLA hardware. |
| Parallel to 1–2 | **P0-004 VLA readiness gate** | This is the final VLA-readiness task ID. Time-box CUDA/LeRobot/manipulator/camera/budget `GO`/`NO-GO`; no Dataset V1 or robot motion. |
