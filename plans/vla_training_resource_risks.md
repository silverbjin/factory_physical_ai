...
| `VLA-TR-R10` | Evidence could be manipulated by coordinated edits to material fields, checks, blockers, decision, and payload hash. | Verifier v1.2 rejects placeholder identities, requires exact mode roles and primary-budget linkage, requires a concrete fallback, cross-checks training flags, and forbids `PASS` with `NOT_VERIFIED`; rehashed regressions pass. | False `TRAINING_RESOURCE_READY` could be emitted. | `RESOLVED` | P0-007 verifier/tests; retain as regression invariant |
