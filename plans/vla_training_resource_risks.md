...
| `VLA-TR-R10` | Evidence could be manipulated by coordinated edits to material fields, checks, blockers, decision, and payload hash. | Verifier v1.1 independently validates mode-specific compute, storage, policy-specific budget/cost, fallback, provenance/source, and blocker projections; rehashed coordinated-tamper regressions pass. | False `TRAINING_RESOURCE_READY` could be emitted. | `RESOLVED` | P0-007 verifier/tests; retain as regression invariant |
