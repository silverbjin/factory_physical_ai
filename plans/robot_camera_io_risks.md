...
| DEV-IO-R09 | Device discovery or frame/state probes could otherwise hang or lose a completed worker result through unreliable queue polling. | Fixed `/dev` patterns use a 5 s child-process bound; state metadata/stat/read share one 2 s bound; camera uses 5 s/1 frame in the canonical run. State/camera use bounded queue retrieval plus explicit process/queue cleanup; delayed-stat and completed-worker tests PASS. | A diagnostic could leak, block, or conservatively misclassify the readiness workflow. | `RESOLVED` | Maintainer; keep bounds, queue cleanup, and timeout/result regression tests |
...
| DEV-IO-R15 | Pre-commit evidence could be mistaken for evidence generated from a later commit. | `content_binding` records `generation_mode`, generation-time Git HEAD/dirty state, bound source hashes, verifier hash, and evidence payload hash. | Ambiguous provenance could undermine auditability after the remediation commit. | `RESOLVED` | Maintainer; preserve content binding on every canonical regeneration |
