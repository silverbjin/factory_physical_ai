...
def main() -> int:
    ...
    for name, profile in PROFILES.items():
        ...
        rows.append({"profile": name, "pass": mission["result"] == "success" and outcome.get("verification", {}).get("verdict") == "pass" and runtime.cleaned_up, "mission_id": mission["mission_id"], "trace_id": mission["trace_id"], "action_ids": [outcome.get("navigation", {}).get("action_id"), outcome.get("vla", {}).get("action_id"), outcome.get("verification", {}).get("action_id")], "mission_result": mission["result"], "failure_code": mission.get("error", {}).get("code"), "system_world": profile.integrated_world, "mujoco_live_world": name != "system" and profile.vla == "mujoco", "verification_before_completion": "verification" in outcome, "lifecycle": runtime.lifecycle, "cleanup_complete": runtime.cleanup_complete})
